...
class CommandParser:
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        # Process command
        process_parser = subparsers.add_parser('process', help='Process image with effects')
    ...
    def _add_common_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument('--random', action='store_true', help='Use random image from library')
        ...
        # Output parameters
        parser.add_argument('--format', choices=['PNG', 'JPEG', 'GIF'], help='Output format')
        parser.add_argument('--quality', type=int, choices=range(101), help='Output quality (0-100)')
    ...
    def parse_command(self, command_str: str, 
        ...
        for effect in self.config.effect_params:
            if hasattr(args, effect) and getattr(args, effect) is not None:
                ...
                if len(values) == 1:
                    params = {'intensity': values[0]} if 'intensity' in self.config.effect_params[effect] else {'value': values[0]}
                else:
                    params = {'intensity': tuple(values[:2])} if 'intensity' in self.config.effect_params[effect] else {'values': values}
        ...
        if args.command == 'animate':
            ...
            if not (self.config.animation.min_frames <= args.frames <= self.config.animation.max_frames):
                raise ValueError(f"Frames must be between {self.config.animation.min_frames} and {self.config.animation.max_frames}")
            if not (self.config.animation.min_fps <= args.fps <= self.config.animation.max_fps):
    ...
    def _validate_command(self, parsed: ParsedCommand) -> None:
        ...
        if not parsed.image_path and not parsed.preset_name and not any('--random' in effect for effect in parsed.effects):
