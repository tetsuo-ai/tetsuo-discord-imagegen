...
class ASCIIProcessor:
    ...
    def convert_to_ascii(
        self, image: Image.Image, cols: Optional[int] = None, scale: Optional[float] = None, 
        moreLevels: bool = False
    ) -> List[str]:
        """Convert single image/frame to ASCII art"""
        if cols is None or scale is None:
            cols, scale = self.get_optimal_dimensions()
    ...
    def ascii_to_image(self, ascii_art: List[str], background_color=(255, 255, 255), text_color=(0, 0, 0)) -> Image.Image:
        """Convert ASCII art string to PIL Image"""
        # Split the ASCII art into lines

        # Calculate image size based on font metrics
        char_width, char_height = self.get_char_size('A')  # Get size of a single character
        padding = 20  # Add some padding around the text

        # Calculate image dimensions
        image_width = max(len(line) * char_width for line in ascii_art) + padding * 2
        image_height = len(ascii_art) * char_height + padding * 2
    ...
    def convert_to_ascii_image(self, image: Image.Image, cols: int = 80, scale: float = 0.43, 
        ...
        return self.ascii_to_image(ascii_art, background_color, text_color)
