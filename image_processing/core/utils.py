...
class ImageUtils:
    ...
    def get_image_stats(image: Image.Image) -> Dict[str, Any]:
        ...
        color_distribution = [
            sum(hist[i : i + 256]) / total_pixels for i in range(0, 768, 256)
        ]
    ...
    def interpolate_value(
        start: ParamValue, end: ParamValue, progress: float
    ) -> ParamValue:
        """
        ...
        if isinstance(start, (tuple, list)) and isinstance(end, (tuple, list)):
            ...
            return type(start)(
                start[i] + (end[i] - start[i]) * progress for i in range(len(start))
            )
